//Here we are going to write a C program for the Shortest Job first scheduling algorithm in Non-premptive mode.....
    ...
    while(completed!=n){
        ...
        for(int i=0;i<n;i++){
            if(pro[i].at<=current&&is_remaining[i]!=true){
                if(pro[i].bt<Smallest_bt){
                    index=i;
                    Smallest_bt=pro[i].bt;
                }
                else if(pro[i].bt==Smallest_bt&&pro[i].at<pro[index].at){
                    index=i;
                }
