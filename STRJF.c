//Here we are going to write a C program for the Shortest job first scheduling algorithm in premptive mode.....
...
int main(){
    ...
    int index=-1,Smallest_bt=INT_MAX,Start_time=0;
    while(completed!=n){
        index=-1;
        Smallest_bt=INT_MAX;
        for(int i=0;i<n;i++){
            if(pro[i].at<=current&&is_remaining[i]!=true){
                if(remaining_bt[i]<Smallest_bt){
                    index=i;
                    Smallest_bt=remaining_bt[i];
                }
                else if(remaining_bt[i]==Smallest_bt&&pro[i].at<pro[index].at){
                    index=i;
                }
        ...
        if(pro[index].bt==remaining_bt[index]){
