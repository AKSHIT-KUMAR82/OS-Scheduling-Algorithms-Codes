//Here we are going to write a C program for the Longest job first scheduling algorithm in premptive mode.....
    ...
    while(completed!=n){
        ...
        for(int i=0;i<n;i++){
            if(pro[i].at<=current&&is_remaining[i]!=true){
                if(remaining_bt[i]>Largest_bt){
                    index=i;
                    Largest_bt=remaining_bt[i];
                }
                else if(remaining_bt[i]==Largest_bt&&pro[i].at<pro[index].at){
                    index=i;
                }
